...
- **Fitting performance of the model.** The total loss of the model is satisfyingly low, meaning that the training data has been well fitted.
- **Neither Overfitting nor Under-Fitting.** The epoch number and the learning rate is fine-tuned so that the training stops at their convergence most of the time.
- Third, due to our fine setting of learning rate, there isn't much fluctuation during the training process. The epoch-loss curves are all smooth.
...
$$\text{OFF}_i=\tanh{\Big(\frac{\text{te}_i-\text{tr}_i}{\text{te}_i}\Big)}\quad\Rightarrow \text{OFF}_i=\tanh{(\text{te}_{i-1}-\text{te}_i)}$$
$$
...
The model remained slightly "under fitting".
