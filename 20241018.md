...
### 1. `ExclusiveNet`: An exclusive approach
...
However, we found that the amount of possible postures for "not using" is quit large! A person could be of any pose when not using a cell-phone. Therefore, we are seeking an *exclusive* approach. We tried to build a new neural network named `ExclusiveNet`, which ought to classify an input to one known class or "unknown" if it finds that the $\max\limits_{i\in1,2,\cdots,c}\hat{P}(\omega_i|\mathbf{input}) < threshold$ .
