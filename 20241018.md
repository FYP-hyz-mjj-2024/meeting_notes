...
Usually, a high-dimensional data sample classification with neural network reflects a prerequisite that the natural label of data should be balanced. That is, the amount/situation of data samples may not be too various, like MNIST, where the natural occurrence of number digits are quite equal.

However, we found that the amount of possible postures for "not using" is quit large! A person could be of any pose when not using a cell-phone. Therefore, we are seeking an *exclusive* approach. We tried to build a new neural network named `ExclusiveNet`. This network will classify different types of "Using" data, and leave a Non-confident area for data that doesn't look like any of the types of using data.
![meetingnotes_1018_ExclusiveNet.png](https://s2.loli.net/2024/10/18/32Hbzws9KSALxpO.png)

The non-confident area is defined by a manual threshold. Built-upon the original decision boundary, a data is considered "unknown" if `ExclusiveNet` finds that $\max\limits_{i\in1,2,\cdots,c}\hat{P}(\omega_i|\mathbf{input}) < threshold$ .
...
### 2. About the method on data collection
