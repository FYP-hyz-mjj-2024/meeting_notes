...
- For pre-processing, we fold our original $572$-length vector into a $2$ channeled $2\times 286$ matrix input to convolute the left & right corners with the middle part.
- Then, we immediately use the convolutional layers to convolute the original input into an $8$-channeled one with kernel size of $3$ and padding of $1$, without length reduction. More process is given below.
